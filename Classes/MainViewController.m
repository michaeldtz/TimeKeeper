...
@synthesize squareView,listView,adBannerView;
...
-(void)viewWillAppear:(BOOL)animated{
	self.navigationItem.title = @"TimeKeeper";
...
-(void)askForType{
	UIActionSheet* sheet = [[UIActionSheet alloc] initWithTitle:NSLocalizedString(@"What kind of recording item would you like to create?", @"") delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel", @"") destructiveButtonTitle:nil otherButtonTitles:NSLocalizedString(@"Project",@""), NSLocalizedString(@"Service Order",@""), NSLocalizedString(@"Opportunity",@""), NSLocalizedString(@"Marketing",@""), NSLocalizedString(@"Innovation",@""), NSLocalizedString(@"Others", @""), nil];
...
clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex != 6) {
		...
		tvController.itemType = buttonIndex;
...
-(void)refreshSquarePane{
	...
	for(int i = 0; i < 9; i++){
		if(i >= [itemList count]){
			...
			typeLabel.text     = [PersonalRecordingItem text4ItemType:team.type];
			typeLabel.hidden   = NO;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	cell.detailTextLabel.text = [PersonalRecordingItem text4ItemType:item.type];
